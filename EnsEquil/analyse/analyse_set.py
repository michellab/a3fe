...
def compute_stats(all_results: _pd.DataFrame) -> _Dict[str, _List[float]]:
    ...
    n_bootstrap = 1000
    bootstrapped_exp_dg, bootstrapped_calc_dg = get_bootstrapped_results(all_results=all_results,
                                                                         n_bootstrap=n_bootstrap)

    # For each metric, calculate i) over the actual data ii) overall bootstrapped data and extract stats
    for metric in results:
        results[metric].append(compute_statistic(all_results["exp_dg"], all_results["calc_dg"], metric))
        bootstrapped_metric = _np.zeros([n_bootstrap])
        for i in range(n_bootstrap):
            bootstrapped_metric[i] = compute_statistic(bootstrapped_exp_dg[i], bootstrapped_calc_dg[i], metric)
...
def get_bootstrapped_results(all_results: _pd.DataFrame,
                             n_bootstrap: int = 1000) -> _Tuple[_np.ndarray, _np.ndarray]:
    """
    ...
    if len(exp_dg) != len(calc_dg):
        raise ValueError("The lengths of the calculated and experimental free energy values must match")
    ...
    for i in range(n_bootstrap):
        ...
        bootstrapped_exp_dg[i] = _np.array([_np.random.normal(loc=exp_dg[i], scale=exp_sem[i]) for i in indices])
        bootstrapped_calc_dg[i] = _np.array([_np.random.normal(loc=calc_dg[i], scale=calc_sem[i])for i in indices])

    return bootstrapped_exp_dg, bootstrapped_calc_dg

def compute_statistic(exp_dg: _pd.Series,
                      calc_dg: _pd.Series,
                      statistic: str) -> float:
    """
