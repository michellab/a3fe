...
Adaptive simulation time allocation and automatic equilibration detection are not currently supported with GROMACS.
...
One weakness of a3fe is that its default molecular dynamics engine for production simulations (SOMD) does not support enhanced sampling; HREX is not available. However,
this does mean that all individual SOMD simulations can be run in parallel.
GROMACS can also be used for non-adaptive production simulations, but HREX has not been implemented for GROMACS in a3fe.
